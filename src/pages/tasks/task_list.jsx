...
const TasksList = ({ children }) => {
    ...
    return (<>
            ...
            return (<KanbanItem key={task.id} id={task.id} data={{ ...task, stageId: "unassigned" }}>
            ...
            return (<KanbanColumn key={column.id} id={column.id} title={column.title} count={column.tasks.length} onAddClick={() => handleAddCard({ stageId: column.id })}>
                                {isLoading && <ProjectCardSkeleton />}
                                {!isLoading &&
                    column.tasks.map((task) => {
                        return (<KanbanItem key={task.id} id={task.id} data={task}>
                                                <ProjectCardMemo {...task} dueDate={task.dueDate || undefined}/>
                                            </KanbanItem>);
                    })}
                                {!column.tasks.length && (<KanbanAddCardButton onClick={() => handleAddCard({
                        stageId: column.id,
                    })}/>)}
                            </KanbanColumn>);
        })}
...
const PageSkeleton = () => {
    ...
    return (<KanbanBoardContainer>
            ...
            return (<KanbanColumnSkeleton key={index}>
